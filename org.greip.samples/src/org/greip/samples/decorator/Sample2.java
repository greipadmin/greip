...
import org.greip.decorator.NumberDecorator;
import org.greip.decorator.PercentageDecorator;
import org.greip.samples.AbstractSample;

/**
 * The example shows the use of an {@link NumberDecorator}. A
...
public class Sample2 extends AbstractSample {
	...
	protected void layout() {
		...
		// create and configure the decorator
		final NumberDecorator decorator = new NumberDecorator(composite);
		decorator.setValue(1234.5);
		decorator.setValueColor(display.getSystemColor(SWT.COLOR_GREEN));
		decorator.setNumberFormat(NumberFormat.getInstance());
		decorator.setFont(font);
		decorator.setOuterDiameter(80);
		decorator.setInnerDiameter(60);
		decorator.setCircleColor(display.getSystemColor(SWT.COLOR_DARK_GREEN));
